...
const App = () => {
	...
	const themeOptions = state?.themes.map((theme, index) => (
		<option key={index} value={index} className="dark:bg-neutral-950" style={{ color: theme?.colors?.word }}>
			{theme?.name}
		</option>
	));
	...
	return (
		<main className="font-rubik bg-neutral-100 text-neutral-800 dark:bg-neutral-950 dark:text-neutral-200 p-3 px-5 grid gap-3 relative overflow-hidden max-w-[15rem]">
			<span
				className={
			...
			{isLoading ? (
				...
				<form className="grid gap-3 z-10 select-none">
					...
					<div className={"duration-300 grid gap-1 " + (!state.active && "opacity-30")}>
						...
						<p className="text-neutral-500 text-[10px] text-center">{state.themes[state.themeId]?.description}</p>
					</div>

					<div className="pt-3 duration-300" style={{ opacity: !state.active ? 0.3 : 1 }}>
			...
			<footer className="mt-2">
				<p className="text-[9px] text-center font-semibold text-neutral-500">
					Made with ❤️ by{" "}
					<a title="Jairaume" href="https://www.rasclejero.me" className="underline hover:text-[--accent-color]" target="_blank">
						jairaume
					</a>
				</p>
			</footer>
